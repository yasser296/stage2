    ...
    try:
        ...
        for root, _, files in os.walk(temp_dir):
            for fname in files:
                    ...
                    with open(new_path, encoding="utf-8") as f:
                        content = f.read()
                        msgids.extend(re.findall(r"&lt;pacs:MsgId&gt;(.*?)&lt;/pacs:MsgId&gt;", content))
        return [mid.strip() for mid in msgids]
    finally:
        # Nettoyage du dossier temporaire
        shutil.rmtree(temp_dir)
...
def compare_and_save(D_messages, S_messages, msgids, output_dir, chemin_fichier_S):
    ...
    # --- Fichiers de sortie ---
    #with open(os.path.join(output_dir, "doublons.txt"), "w", encoding="utf-8") as f:
    #    f.write("=== Doublons globaux (tous D) ===\n")
    #    for bloc, paths in global_source_map.items():
    #        if len(paths) > 1:
    #            bloc2, bloc4 = bloc
    #            f.write(f"Bloc2: {bloc2}\nBloc 4:\n{bloc4}\n")
    #            f.write("Sources:\n" + "\n".join(paths) + "\n\n")
    ...
    #with open(os.path.join(output_dir, "rapport_complet.txt"), "w", encoding="utf-8") as f:
        #f.write(f"Résumé :\n")
        #f.write(f"Total messages D  = {len(D_messages)}\n")
        #f.write(f"Total messages S = {len(S_messages)}\n")
        #f.write(f"Doublons filtrés (bloc2=I103-202-200-700) = {len(duplicates)}\n")
        #f.write(f"Doublons globaux (tous D) = {len(global_duplicates)}\n")
        #f.write(f"Absents = {len(missing_in_S)}\n\n")
        #for bloc4, bloc2, path in D_messages:
        #    if bloc4 in duplicates:
        #        status = "DOUBLON"
        #    elif bloc4 in missing_in_S:
        #        status = "ABSENT"
        #    elif bloc4 in matched_blocs:
        #        status = "MATCH"
        #    else:
        #        status = "OK"
        #    f.write(f"Statut: {status}\nSource: {path}\nBloc2: {bloc2}\nBloc 4:\n{bloc4}\n\n")

    #with open(os.path.join(output_dir, "rapprochement_msgid_field20_32A_MT700.txt"), "w", encoding="utf-8") as f:
...
chemin_fichier_S = r"C:/Users/BOURJAS/Documents/Documentation/Rapprochement/Extraction_SAA/flux_S.zip"
repertoire_D = r"C:/Users/BOURJAS/Documents/Documentation/Rapprochement/SO/Repertoire_D"
